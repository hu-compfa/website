...
from .models import Program,Major,SuggAndComp,Level,Courses,Courses_files,Semester,Activities,Teacher,Tutorials,TutorialsTeacher,TutorialsVideos
from .forms import add_program, edit_student,add_student,add_suggestion,create_course,add_files,add_teacher,add_tutorial_teacher,add_tutorial_video,add_activity,add_tutorial,edit_profile,edit_user
...
def teacher_page(request,m_name,level,s_name,c_name):
	...
	for teacher in course.teacher_set.all():
		if teacher.course.get(course_name=course.course_name).courses_files_set.count() != 0:
...
def courses_file_page(request,m_name,level,s_name,c_name,t_name):
	...
	level = get_object_or_404(Level, level=level)
	semester = get_object_or_404(Semester, semester=s_name)
	course = get_object_or_404(Courses, slug=c_name.replace(' ','-'), major__major_name=major.major_name,level__level=level.level)
...
def add_page_for_teacher(request):
		...
		if 'lecture' in request.POST:
			...
			if lecture_form.is_valid():
				...
				for lecture in lectures:
					if Teacher.objects.get(id=teacher).courses_files_set.filter(lecture_name=lecture):
						teacher_have += "  " + lecture.name
				if len(teacher_have) > 0:
					messages.warning(request, f'{teacher_have} are upload')
					return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
				elif Courses.objects.get(id=course) in Teacher.objects.get(id=teacher).course.all():
					for file in lectures:
						form = Courses_files(lecture_url=file, course_id=course, teacher_id=teacher,student_id=request.user.student.id)
						form.save()
					messages.success(request, 'Add lecture/s successful')
					return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
				else:
					Teacher.objects.get(id=teacher).course.add(Courses.objects.get(id=course))
					for file in lectures:
						form = Courses_files(lecture_url=file, course_id=course, teacher_id=teacher,student_id=request.user.student.id)
						form.save()
					messages.success(request, 'Add lecture/s successful')
	...
	context={
		...
		'courses':Courses.objects.all(),
		'levels':Level.objects.all(),
		'teachers':request.user.student.teacher.all(),
		'nav_path':f'/add/',
	}
	...
	if request.method == 'POST':
		if 'register_student' in request.POST:
			...
			if student_form.is_valid() and user_creation_form.is_valid():
				...
				print("bb")
				messages.warning(request,user_creation_form.errors,student_form.errors)
				return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
		...
		if 'add_lecture' in request.POST:
			...
			if lecture_form.is_valid():
				for file in lectures:
					form = Courses_files(lecture_url=file, course_id=course, teacher_id=teacher)
					form.save()
				messages.success(request, 'Add lecture/s successful')
				return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
			else:
...
def tutorial_redirect_page(request,tutorials_name,tutorial_course_name):
	tutorial=get_object_or_404(Tutorials,tutorials_name=tutorials_name)
	tutorial_course=get_object_or_404(TutorialsTeacher,slug=tutorial_course_name.replace(' ','-'))
	video=tutorial_course.tutorialsvideos_set.first()
	return redirect('tutorials_video_page',tutorials_name,tutorial_course_name,video.slug)
...
def tutorial_video_page(request,tutorials_name,tutorial_course_name,video_name):
	...
	current_video=redirect('tutorials_video_page', tutorials_name, tutorial_course_name, video_name.replace(' ','-')).url
...
def profile_page(request):
	...
	context={
		...
		'lectures':request.user.student.courses_files_set.all(),
		'teachers':request.user.student.teacher.all()
	}
...
def edit_profile_page(request):
	...
	update_profile=edit_student(instance=request.user.student)
	change_password=PasswordChangeForm(request.user)
	if request.method== 'POST':
		if 'edit_profile' in request.POST:
			update_user = edit_user(request.POST,instance=request.user)
			update_profile = edit_student(request.POST,instance=request.user.student)
