...
class Major(models.Model):
    ...
    def save(self,*args,**kwargs):
        self.slug=self.major_name.replace(' ','-')
...
class Activities(models.Model):
    ...
    activitie_create_date=models.DateTimeField(default=datetime.datetime.now,null=True,blank=True)
...
class TutorialsTeacher (models.Model):
    ...
    tutorials_image=models.ImageField(null=True,blank=True,upload_to='TutorialsTeacher')
    ...
    def __str__(self):
        return f'{self.slug}'
    def save(self,*args,**kwargs):
        self.slug=f"{self.tutorials_name.replace(' ','-')}-{random.randint(10000,99999)}"
